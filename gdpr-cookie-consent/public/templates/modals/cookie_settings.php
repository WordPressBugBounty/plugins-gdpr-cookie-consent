...
if ( ! defined( 'ABSPATH' ) ) {
	...
	if(!function_exists('gdpr_is_gpc_honored')){
...
<div class="gdprmodal gdprfade" id="gdpr-gdprmodal" role="dialog" data-keyboard="false" data-backdrop="<?php echo esc_html( $cookie_data['backdrop'] ); ?>" >
	<div class="gdprmodal-dialog gdprmodal-dialog-centered">
        ...
        ">
			...
			<div class="gdprmodal-body classic classic-nvg" style="scrollbar-color: <?php echo esc_html( $cookieSettingsPopupAccentColor ); ?> transparent;">
				<div class="gdpr-details-content">
					<div class="gdpr-groups-container">
                 		<?php if ( $the_options['cookie_usage_for']==='gdpr' ) :?>
							...
							<div class="gdpr-about-cookies">
								<?php echo esc_html($the_options['gcm_about_message']); ?>
									<a  style="color: <?php echo $the_options['cookie_usage_for'] == 'ccpa' ?  esc_html($the_options['button_donotsell_link_color' . $suffix]) : ((bool)$the_options['button_accept_all_as_button' . $suffix] === 'true' || (bool)$the_options['button_accept_all_as_button' . $suffix] === true || (bool)$the_options['button_accept_all_as_button' . $suffix] === 1 ? esc_html($the_options['button_accept_all_button_color' . $suffix]) : esc_html($the_options["button_accept_all_link_color" . $suffix]));?>;"
						...
						?>
						<ul class="gdpr-iab-navbar" style="border-bottom: 1px solid <?php echo esc_attr($border_color); ?>;">
										...
										if ( ( $the_options['button_settings_display_cookies' . $chosenBanner] === true || $the_options['button_settings_display_cookies' . $chosenBanner] === 'true' ) ) {
														...
														<?php foreach ( $category['data'] as $cookie ) : ?>
															<div class="cookie-card">
																<div class="cookie-row">
																	<div class="cookie-label"><?php echo esc_html( $cookie_data['name'] ); ?></div>
																	<div class="cookie-value">
																		<?php echo ! empty( $cookie['name'] ) ? esc_html( $cookie['name'] ) : '---'; ?>
																	</div>
																</div>

																<div class="cookie-row">
																	<div class="cookie-label"><?php echo esc_html( $cookie_data['domain'] ); ?></div>
																	<div class="cookie-value">
																		<?php echo ! empty( $cookie['domain'] ) ? esc_html( $cookie['domain'] ) : '---'; ?>
																	</div>
																</div>

																<div class="cookie-row">
																	<div class="cookie-label"><?php echo esc_html( $cookie_data['purpose'] ); ?></div>
																	<div class="cookie-value">
																		<?php echo ! empty( $cookie['description'] ) ? esc_html( $cookie['description'] ) : '---'; ?>
																	</div>
																</div>

																<div class="cookie-row">
																	<div class="cookie-label"><?php echo esc_html( $cookie_data['expiry'] ); ?></div>
																	<div class="cookie-value">
																		<?php echo ! empty( $cookie['duration'] ) ? esc_html( $cookie['duration'] ) : '---'; ?>
																	</div>
																</div>

																<div class="cookie-row">
																	<div class="cookie-label"><?php echo esc_html( $cookie_data['type'] ); ?></div>
																	<div class="cookie-value">
																		<?php echo ! empty( $cookie['type'] ) ? esc_html( $cookie['type'] ) : '---'; ?>
																	</div>
																</div>
															</div>
														<?php endforeach; ?>
											...
											if ( ( $the_options['button_settings_display_cookies'] === true || $the_options['button_settings_display_cookies'] === 'true' ) ) {
												...
												<div class="category-cookies-list-container">
													...
													if ( $category['total'] >= '1' ) {
														?>
														<div class="cookie-list" style="background: <?php echo esc_attr( $overlay_color ); ?>; border: 1px solid <?php echo esc_attr( $border_color ); ?>">
															<?php foreach ( $category['data'] as $index => $cookie ) : ?>
																<div class="cookie-card">
																	<div class="cookie-row">
																		<div class="cookie-label"><?php echo esc_html( $cookie_data['name'] ); ?></div>
																		<div class="cookie-value">
																...
																<?php if ( $index < count( $category['data'] ) - 1 ) : ?>
																	<hr class="cookie-divider" style="border-top: 1px solid <?php echo esc_attr( $border_color ) ?>;">
																<?php endif; ?>
															<?php endforeach; ?>
														</div>
														<?php
			...
			<div class="gdprmodal-footer" style="--backgorund-color-popup: <?php echo esc_html( '#' . ltrim($finalColor, '#') ); ?>; --popup_accent_color: <?php echo esc_html( '#' . ltrim($cookieSettingsPopupAccentColor, '#') ); ?>; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
				<?php if ( gdpr_is_gpc_honored( $the_options ) ) : ?>
					<p class="gdpr-gpc-site-honored">	
				...
				<div class="gdpr-footer-left" style="display:flex; gap:10px; flex-wrap:wrap;">
					...
					<button id="cookie_action_reject" type="button" class="gdpr_action_button btn" data-gdpr_action="reject" data-dismiss="gdprmodal"
						...
						padding: 12px 29px; text-transform: none; font-weight:400;
				...
				if ( ! empty( $cookie_data['show_credits'] ) ) {
					?>
				<div class="powered-by-credits" style="margin-left: <?php echo esc_attr( $top_value ); ?>px;">
